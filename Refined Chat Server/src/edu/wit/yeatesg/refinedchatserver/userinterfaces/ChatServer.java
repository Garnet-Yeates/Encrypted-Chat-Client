...
public class ChatServer extends Console
{
	...
	protected void onSend(String consoleMessage)
	{
		...
		{
		case "/end": case "/exit": case "/quit": // End all connections case
			exitAllClients();
			terminate(1500);
			break;
		default:
			if (consoleMessage.length() >= 7 && consoleMessage.substring(0, 6).toLowerCase().equals("/kick ")) // kick command case
			{
				...
				{
					System.out.println("kicking: " + clientKickingName);
					Client kicking = clientList.get(consoleMessage.substring(6));
					MessagePacket youHaveBeenKicked = new MessagePacket(serverName, "You have been kicked from the server!", Color.RED, Color.RED);
					closeConnection(kicking, youHaveBeenKicked);

					MessagePacket youKickedClient = new MessagePacket("Kicked " + clientKickingName + " from the server.", Color.PURPLE, Color.PURPLE);
					logToConsole(youKickedClient); // Server notifies itself that it kicked a client

					MessagePacket clientWasKicked = new MessagePacket(serverName, clientKickingName + " has been kicked from the server.", Color.BLUE, Color.BLUE);
					clientWasKicked.sendMultiple(getAllDataStreams()); // Notify other clients that a client was kicked
				}
			}
			else // In this case the server is just sending a regular ol' message to all the clients
			{
	...
	{		
		...
		// End client thread, remove from clientList, log to console that the connection was closed
